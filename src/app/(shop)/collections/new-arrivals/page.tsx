...
async function fetchNewArrivals(): Promise<Product[]> {
  const res = await fetch("/api/products?newArrivals=true", {
...
export default function NewArrivalsPage() {
  ...
  React.useEffect(() => {
    if (products) {
      console.log("New Arrivals products:", products.length, products);
    }
    if (error) {
  ...
  if (error) {
    return (
      <div className="space-y-6">
        ...
        >
          <h1 className="text-3xl font-bold text-[#333333] md:text-[40px]">New Arrivals</h1>
        </div>
      ...
      <div className="min-w-0 flex-1 space-y-6">
        ...
        >
          ...
          <p className="mt-3 line-clamp-2 text-lg text-[#333333]/80">
            Discover our handpicked new arrivals - fresh styles curated just for you
          </p>
        ...
        {paginated.length === 0 ? (
          <div className="flex flex-col items-center justify-center rounded-xl border border-dashed border-[#ddd] bg-[#F5F3EE]/50 py-20 text-center">
            ...
            {products.length === 0 && !isLoading && (
              <p className="mt-2 text-sm text-[#333333]/60 max-w-md">
                No new arrival products available. Make sure products are marked as "New Arrival" and have "ACTIVE" status in the admin dashboard.
              </p>
            )}
